...
.. _whatsnew_100.duplicates:

Ensuring unique labels
~~~~~~~~~~~~~~~~~~~~~~

:class:`Series` and :class:`DataFrame` now accept a keyword to control whether
duplicate labels are allowed (:issue:`27108`). By default, duplicates are allowed (which is
compatible with previous versions of pandas).

.. ipython:: python
   :okexcept:

   pd.Series([1, 2], index=['a', 'a'], allow_duplicate_labels=False)

The property may be checked

.. ipython:: python

   s = pd.Series([0, 1], index=['a', 'A'], allow_duplicate_labels=True)
...
The property persists through most pandas operations. If an operation introduces
duplicate labels, an exception is raise.

.. ipython:: python
   :okexcept:

   s.rename(index=lambda x: x.upper())
...
- Metadata is now finalized for the following methods on ``Series`` and ``DataFrame`` (:issue:`28394`)
   * :meth:`~DataFrame.abs`
